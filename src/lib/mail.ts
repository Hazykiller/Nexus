...
export async function sendOtpEmail(email: string, otp: string, tier: string) {
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; color: #333;">
            <h1 style="color: #0d9488;">Vertex Security</h1>
            <p>Your 6-digit security code is:</p>
            <div style="background: #f4f4f4; padding: 20px; text-align: center; font-size: 32px; font-weight: bold; letter-spacing: 5px; color: #0d9488;">
              ${otp}
            </div>
            <p>This code expires in 15 minutes.</p>
            <p style="font-size: 12px; color: #999;">Account Tier: ${tier}</p>
          </div>
        `,
      }),
    });
    ...
    console.error('Vertex Mailer Failure:', error);
